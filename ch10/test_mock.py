...
def test_mock_CardsDB():
    with mock.patch('cards.cli.cards.CardsDB') as MockCardsDB:
        print()
...
def test_bad_mock():
    with mock.patch('cards.cli.cards.CardsDB') as CardsDB:
...
def test_good_mock():
    with mock.patch('cards.cli.cards.CardsDB',
                    autospec=True) as CardsDB:
        db = CardsDB('/some/path')
        db.path() # good
        db.path(35)  # invalid arguments
        db.not_valid()  # invalid function
